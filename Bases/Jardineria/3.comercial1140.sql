...
insert into pedidos (codigopedido,fechapedido,fechaesperada,fechaentrega,estado,comentarios,codigocliente) values ('130','06/02/2023','10/02/2023','','Pendiente','','28');
lock table detallepedidos in exclusive mode;
insert into detallepedidos (codigopedido,codigoproducto,cantidad,preciounidad,numerolinea) values ('130','AR-001','140','1','3');

lock table productos in exclusive mode;
update productos set cantidadenstock = (select cantidadenstock from productos where lower (nombre) = 'ajedrea') - (select cantidad from detallepedidos where codigopedido in '130') where lower(nombre) in 'ajedrea'
